import {
  little_endian_bytes_to_num,
  NumToByteConverter,
  unsigned_number_would_overflow,
} from "./utils";
...
export function build_midi_files_from_midi_event_builder(
  midi_builder: MidiEventBuilder
) {
...
function get_total_length_of_track_events(
  track_events: MidiTrackEvent[]
): number {
  const total_length = track_events.reduce((length, event) => {
    return (
      length +
      event.event.length +
      NumToByteConverter.variable_length(event.v_time).length
    );
  }, 0);
  return total_length;
}
...
export function make_midi_header_chunk(
  ...
  if (
    !is_valid_amount_of_tracks_for_file_format(file_format, number_of_tracks)
  ) {
  ...
  if (unsigned_number_would_overflow(ticks_per_quarter_note, 15)) {
    throw new Error(
      "Ticks per quarter note would overflow the maximum 15 bits"
    );
  }
...
export function make_midi_track_chunk(
  track_events: MidiTrackEvent[]
): Uint8Array {
